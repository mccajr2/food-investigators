...
- [x] A new parent can **register** with email + password (web and via API) and
      receives a session token; a **household** is created for that account.
- [x] An existing parent can **sign in** with email + password on **web** and
      **iOS** and receive a session token.
- [x] Sign-in UI offers a **Keep me logged in** checkbox (default on).
- [x] With **Keep me logged in checked**, web and iOS stay signed in across
      browser reload / app restart until sign-out.
- [x] With **Keep me logged in unchecked**, the token is not restored after
      browser session end / app process end; user must sign in again.
- [x] **Sign out** invalidates the server session; subsequent authenticated calls
      with that token fail (401).
- [x] `GET /api/auth/me` with a valid token returns the current user’s id, email,
      and household id; without a token (or with a bad token) returns 401.
- [x] Duplicate email on register returns a clear client error (e.g. 409), not a
      500.
- [x] Wrong password / unknown email on login returns 401 without leaking which
      field failed.
- [x] `contracts/openapi.yaml` documents auth endpoints only (no greeting path);
      web and mobile clients match the contract in the same change.
- [x] Greeting harness is gone: no `greeting` backend module, no greeting API
      clients/UI on web or mobile; README smoke uses auth.
- [x] Unit + integration tests cover register, login (both remember modes),
      logout, me, duplicate email, and unauthorized me; `ModularityTests` still
      passes.

## Tasks

- [x] Backend: Add persistence + Flyway (or chosen migrator) and Spring Security
      wiring at the app level as needed for the `accounts` module (confirm deps
      in version catalog first).
- [x] Backend: Create `backend/modules/accounts/` with register/login/logout/me,
      password hashing, household creation, session token store (TTL differs for
      remember vs not), and Modulith `internal` boundaries.
- [x] Backend: Remove `backend/modules/greeting/` and its tests.
- [x] Contract: Replace greeting with auth paths/schemas in
      `contracts/openapi.yaml` (product-oriented API title/description).
- [x] Web: Remove greeting harness; auth API client + register/sign-in/sign-out
      + Keep me logged in; show minimal “signed in as …” shell (no tasting UI).
- [x] iOS: Remove greeting UI/client usage; sharedLogic auth client + token
      storage honoring remember flag; SwiftUI sign-in/sign-out + Keep me logged
      in; show signed-in state (no tasting UI).
- [x] Docs: Update README quick start / smoke to auth instead of greeting.
- [x] Tests: Module unit tests + API integration tests for the AC above;
