...
6. You will see a URI like:
   ...
   ```text
   SPRING_DATASOURCE_URL=jdbc:postgresql://ep-cool-name-12345.us-west-2.aws.neon.tech/neondb?sslmode=require
...
- [ ] Neon project created; region noted
- [ ] `SPRING_DATASOURCE_*` set on Render (secrets)
- [ ] `GEMINI_API_KEY` set on Render
- [ ] Service URL recorded: `https://________________.onrender.com`
- [ ] `curl …/actuator/health` → `{"status":"UP"}`
- [ ] UptimeRobot monitor every 5 minutes on that health URL → Up
